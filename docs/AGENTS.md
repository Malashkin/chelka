...
- Перетащить файл на чёлку — он падает на полку (`~/Shelf`) и, если задан `peerHost`,
  уезжает на вторую машину в такую же папку. Синхронизация двусторонняя, если
  `peerHost` задан на обеих машинах (README, шаг «в обе стороны»); петля
  невозможна — пушатся только файлы, задропленные на этой машине.
- Файл, задропленный до настройки `peerHost`, досылается через контекстное
  меню («Отправить на вторую машину»).
