...
Предполагается, что обе машины принадлежат **одному человеку**: транспортный
ключ даёт обычный ssh-доступ к пиру, а прилетающие от него файлы считаются
доверенными (без quarantine). Значение `peerHost` валидируется до попадания
в ssh/rsync (инъекция опций/shell исключена), передача никогда не
перезаписывает существующие файлы на приёмнике, а само приложение удаляет
только в Корзину. Полный аудит — модель угроз, находки, остаточные риски —
в [docs/security/index.md](docs/security/index.md).
