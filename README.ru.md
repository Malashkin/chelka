...
- Полка — прозрачная панель поверх чёлки (`NSPanel`, не перехватывает фокус).
  На экране без чёлки (Mac mini, внешний монитор) — полоска по центру, чуть ниже кромки (чтобы в неё можно было попасть drag'ом даже внутри окна Screen Sharing; отступ настраивается: `defaults write dev.mike.Chelka fallbackTopOffset <pt>`).
